...
def parse_args():
    ...
    parser.add_argument("--output", type=Path, default=v2_config.V2_TIMELINE_GIF)
    ...
    parser.add_argument("--daily-output-dir", type=Path, default=v2_config.V2_DAILY_ANIMATION_OUTPUT_DIR)
...
def load_rows(path: Path):
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            row["role"] = "person" if row["label"].strip().lower() == v2_config.PERSON_LABEL else "machine"
            rows.append(row)
    return rows
...
def eligible_object_ids(rows, role, min_observations):
...
def build_forward_filled_machine_frames(timestamps, frame_rows, eligible_machine_ids):
    ...
    for timestamp in timestamps:
        ...
        people_rows = [row for row in current_rows if row.get("role") == "person"]
        current_machine_rows = {}
        for row in current_rows:
            if row.get("role") != "machine":
                continue
...
def subset_histories(rows, role, min_observations, temperature_column):
    # build histories for one object category.
    subset = [row for row in rows if row.get("role") == role]
    counts = defaultdict(int)
    for row in subset:
        counts[row["object_id"]] += 1
...
def build_animation_from_rows(rows, args):
    ...
    if not rows:
        raise ValueError("No rows found for animation.")
