...
from config import XGBOOST_MULTIHORIZON_DIR, OBJECT_TIMELINE_CSV, ensure_output_dirs
...
def save_forecast_outputs(models, predictions, metrics, feature_columns, args, evals_result):
    ...
    for name, model in models.items():
        model.save_model(str(args.output_dir / f"xgboost_multihorizon_{name}.json"))
...
def save_group_forecast_outputs(
    ...
    group_metrics_csv = args.output_dir / "xgboost_multihorizon_group_metrics.csv"
    ...
    for model_group, models in group_models.items():
        ...
        for name, model in models.items():
            model.save_model(str(group_dir / f"{name}.json"))
...
def run_grouped_multihorizon_forecast_pipeline(args):
    ...
    if not skipped_groups.empty:
        skipped_groups.to_csv(args.output_dir / "xgboost_multihorizon_skipped_groups.csv", index=False)
...
def default_multihorizon_args():
    return SimpleNamespace(
        ...
        prediction_csv=XGBOOST_MULTIHORIZON_DIR / "xgboost_multihorizon_predictions.csv",
        metrics_csv=XGBOOST_MULTIHORIZON_DIR / "xgboost_multihorizon_metrics.csv",
        feature_importance_csv=XGBOOST_MULTIHORIZON_DIR / "xgboost_multihorizon_feature_importance.csv",
        learning_curve_png=XGBOOST_MULTIHORIZON_DIR / "xgboost_multihorizon_learning_curve.png",
